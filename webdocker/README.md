...
2b) If your uid and/or gid is different than 1000 you have to correct them in the __Dockerfile__ near the end of the file. (Look for `# Make webserver run with`).
 Replace the 1000 in `addgroup --gid 1000 user` and `usermod -G 1000 www-data` with your group id. Also replace the 1000 in `usermod -u 1000 www-data` with your user id.
    This assures that files created by the webserver are accessable on your local machine.

3)Build the webdocker image by running the dockerbuild.sh script

4)copy/edit the projectStart script according your needs

5)run the projectStart script to fire up the servers.

To stop the servers just run projectStop.
